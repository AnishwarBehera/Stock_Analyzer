...
# from document_loader import process_uploaded_files  # Use the new function for processing
...
#         # Process uploaded files to extract documents
#         documents = process_uploaded_files(uploaded_files, document_type)

#         # Validate that documents were extracted successfully
#         if not documents:
#             st.error("No valid documents were extracted. Please check the uploaded files.")
#             return None

#         # Choose text splitter based on document type
...
#             if not chunks:
#                 st.warning(f"Document {i + 1} could not be split properly. Skipping.")
#                 continue  
#             split_docs.extend(chunks)

#         # Validate that there are chunks to create embeddings from
#         if not split_docs:
...
#         # Create vector embeddings from split documents
